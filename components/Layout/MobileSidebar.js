...
const MobileSidebar = ({ visibility, onCloseButtonClicked }) => {
  ...
  return (
    <div
      className={`fixed z-50 inset-0  lg:hidden bg-black/5 transition-all duration-1000 ${
        visibility ? " -translate-x-[0px]" : " -translate-x-[1024px]"
      }`}
    >
      <div className="w-full h-full flex flex-row">
        <div className="self-stretch flex-1 bg-white pt-5 px-4 flex flex-col">
          <div
            className={`self-end transition-all duration-1000 cursor-pointer ${
            ...
            {sidebardata.map((item, index) => {
              return (
                <div
                  ...
                  style={{
                    backgroundColor:
                      router.pathname == item.path ? "#FAFAFC" : "white",
                  }}
                  onClick={() => {
                ...
                >
                  <div>{item.Icon}</div>
                  <div>
                    <h1 className="text-[#807C7C] text-base font-normal">
                      {item.title}
                    </h1>
                  </div>
                  {router.pathname == item.path && (
                    <div className="absolute -left-[24px] top-0 bottom-0 w-1 bg-[#B3DD07]"></div>
                  )}
                </div>
              );
            })}
        ...
        <div
          className="flex-1 self-stretch hidden sm:block "
          onClick={onCloseButtonClicked}
        ></div>
      </div>
    </div>
